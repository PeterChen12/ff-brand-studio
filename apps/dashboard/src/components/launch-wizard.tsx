...
// Friendly labels for the async launch phases the worker reports via
// run.currentPhase, so the Running button shows what's happening (not a raw
// enum) while the multi-minute pipeline runs.
const LAUNCH_PHASE_LABELS: Record<string, string> = {
  ...
  refine_all_crops: "Refining images",
  lifestyle: "Rendering lifestyle scene",
  composites: "Building infographics",
  banner: "Building hero banner",
  write_slots: "Writing assets",
...
export function LaunchWizard({ mcpUrl: _mcpUrl }: { mcpUrl: string }) {
  ...
  // Current server phase while an async launch runs (drives the progress bar).
  ...
  // Cancels the in-flight launch status poll (on a new launch or unmount).
  ...
  // Reconstruct the rich LaunchResult the ResultPanel renders from the async
  // run snapshot (GET /v1/launches/:id returns the run row + listings). Notes
  // aren't persisted on the run, so they collapse to lastError (if any).
  const buildAsyncResult = useCallback(
    (
      runId: string,
      run: {
        status: string;
      ...
      listingsRaw: Array<Record<string, unknown>>,
    ): LaunchResult => {
      const surfaceRows = listingsRaw.map((raw) => {
        // GET returns drizzle join rows ({ platform_listings, ... }); tolerate
        // a flat shape too.
        const l = ((raw.platform_listings as Record<string, unknown>) ??
          raw) as Record<string, unknown>;
        return {
          surface: String(l.surface ?? ""),
          language: String(l.language ?? ""),
          copy: (l.copy as Record<string, unknown> | null) ?? null,
          rating: (l.rating as Rating) ?? "FAIR",
          iterations: Number(l.iterations ?? 0),
          cost_cents: Number(l.costCents ?? 0),
          grounding: null,
        };
      });
      return {
        ...
        status: run.status as LaunchStatus,
        duration_ms: Number(run.durationMs ?? 0),
        total_cost_cents: Number(run.totalCostCents ?? 0),
        hitl_count: Number(run.hitlInterventions ?? 0),
        notes: run.lastError ? [run.lastError] : [],
        seo:
          seoEnabled && surfaceRows.length > 0
            ? {
                status: "ok",
                total_cost_cents: surfaceRows.reduce(
                  (s, x) => s + x.cost_cents,
                  0,
                ),
  ...
  async function handleLaunch(e: React.FormEvent) {
    ...
    const body = JSON.stringify({
      ...
      platforms,
      dry_run: dryRun,
      // Issue 7+10 — surfaces drives both per-surface SEO and the cost
      // prediction. Worker treats include_seo as derived from surfaces.length
    ...
    try {
      // Dry runs are a fast SEO/cost preview (no image pipeline) — keep them
      // synchronous. Real launches run the multi-minute pipeline async + poll
      // so a long generation can't be killed by an edge/client request timeout
      // (the previous synchronous request blocked the whole run), and so we can
      // show real phase progress.
      if (dryRun) {
      ...
      const enq = await apiFetch<{ run_id: string }>("/v1/launches?async=1", {
        method: "POST",
        body,
      });
      ...
      launchAbortRef.current = ctrl;
      const TERMINAL = ["succeeded", "failed", "hitl_blocked", "cost_capped"];
      let pollFailures = 0;
      while (!ctrl.signal.aborted) {
        ...
        try {
          ...
          }>(`/v1/launches/${enq.run_id}`);
          pollFailures = 0;
          const run = snap.run;
          if (TERMINAL.includes(run.status)) {
            setResult(buildAsyncResult(enq.run_id, run, snap.listings ?? []));
            setLaunchPhase(null);
            if (run.status === "succeeded" || run.status === "hitl_blocked") {
              try {
                const assetData = await apiFetch<{
                  platformAssets: PlatformAssetRow[];
                }>("/api/assets");
                setLaunchAssets(
                  (assetData.platformAssets ?? []).filter(
          ...
          setLaunchPhase(run.currentPhase ?? run.status ?? null);
        } catch (pollErr) {
          if (ctrl.signal.aborted) return;
          ...
          if (pollFailures >= 5) {
            setError(
              "Lost connection while tracking the launch — it's still running. Check the Library in a minute.",
            );
            return;
          }
        }
          ...
          <div className="flex flex-col gap-3 pt-2 border-t ff-hairline">
            ...
            >
              {launching
                ? `${
                    launchPhase
                      ? (LAUNCH_PHASE_LABELS[launchPhase] ?? "Running")
                      : "Running"
